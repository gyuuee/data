...
part_A_train_gt = os.path.join(root,'part_A_final/train_data_15','ground_truth')
...
for gt_path in gt_paths:
    ...
    h5_file_path = gt_path.replace('train_data_15', 'train_data_newmap1').replace('ground_truth', 'ground_truth_test')
    ...
    y_ems_mask_1=[]
    for  i_colomns in range(gt_origin.shape[0]):    #for each row ,get the mask of the value above the means
    ...
    y_ems_count = np.sum(y_ems_mask_1, axis=1)    #count the value above the means   for each row
    # print(y_ems_count)
