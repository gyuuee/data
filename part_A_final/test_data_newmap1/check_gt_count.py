...
def handleImage(filename, tar):
    ...
    # gt_path1 = gt_path.replace('ground_truth_new', '../test_data_15/ground_truth')
    # print(gt_path)
    gt_file1 = h5py.File(gt_path1)
    image_gt1 = np.asarray(gt_file1['density'])

    gt_mat_path=gt_path.replace('ground_truth_new','../test_data/ground_truth').replace('IMG_','GT_IMG_').replace('.h5', '.mat')
